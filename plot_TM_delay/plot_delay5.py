...
def subtract_background(ori_data, stu=1, extend=200):
    """
    数据减去背景
...
def plot_data(data1, data2='0', data3='0', data4='0'):
    ...
    # plot_time_image(data1, xmin, xmax, l_label="Only 400nm 1.6mW")
    plot_time_image(data1, xmin, xmax, l_label="delay:62ps", style="r-")
    
    if len(data2) > 1:
        xmin, xmax = get_xmin_xmax(data2)
        plot_time_image(data2, xmin, xmax, l_label="delay:31ps", style="b-")
    
    if len(data3) > 1:
        xmin, xmax = get_xmin_xmax(data3)
        #plot_time_image(data3, xmin, xmax, l_label="delay:25ps", style="b-")
    
    if len(data4) > 1:
        xmin, xmax = get_xmin_xmax(data4)
        #plot_time_image(data4, xmin, xmax, l_label="delay:-5ps", style="r-")
    
    plt.title("400nm 1.6mW and 800nm 14.4mW ")
    plt.show()
    
    
def main():
    folder = r"G:\Group_Work\Wyatt_Experiment\Quantum_dot\Tm_quantum dots1\800+400double_pulse"
    # file1 = r"\weizhi5\400change 400power\alter\1-58.dat"  # 只有相同功率的紫光
    file1 = r"\weizhi5\400+800change delay\alter\0.dat"
    file2 = r"\weizhi5\400+800change delay\alter\4-5.dat"  # 两束光同时打上的数据,但时间上差的比较多，用来排除加热效应
    file3 = r"\weizhi5\400+800change delay\alter\5-5.dat"  # 两束光同时打上的数据,但时间上差的比较多，用来排除加热效应
    file4 = r"\weizhi5\400+800change delay\alter\8-5.dat"  # 两束光同时打上的数据
    ### 1.加载理论上的数据
    real_data1 = subtract_background(load_dat_data(folder + file1), extend=350)  # 只有相同功率的紫光的数据
    real_data2 = subtract_background(load_dat_data(folder + file2))  # 两束光都打上的数据
    
    ### 2.构建理论的数据
    # 移动紫光位置
    # real_data1 = move_data_time(real_data1, cur_pos=61, pur_pos=61)
